...
public:
    ...
    int orient_target = 0; //90 degrees
    ...
    switch (cmd_type) {
        ...
        case GET_PID:
        {
          for (int i = 0; i < mycar.e_pos; i++) {
                ...
                tx_estring_value.append("Motor1:");
                tx_estring_value.append(mycar.motor_hist[i][0]);
                tx_estring_value.append("Motor2:");
                tx_estring_value.append(mycar.motor_hist[i][1]);
...
float calc_pid(int dist, int setpoint, float KP = mycar.SKP, float KI = mycar.SKI, float KD = mycar.SKD, int mode = 0, float alpha = 0.002) {
  ...
  if (mycar.e_pos >= PID_LENGTH) { 
    ...
    mycar.motor_hist[0][0] = mycar.motor_hist[PID_LENGTH-2][0];
    mycar.motor_hist[1][0] = mycar.motor_hist[PID_LENGTH-1][0];
    mycar.motor_hist[0][1] = mycar.motor_hist[PID_LENGTH-2][1];
    mycar.motor_hist[1][1] = mycar.motor_hist[PID_LENGTH-1][1];
    mycar.target_hist[0][0] = mycar.target_hist[PID_LENGTH-2][0];
    mycar.target_hist[1][0] = mycar.target_hist[PID_LENGTH-1][0];
    mycar.target_hist[0][1] = mycar.target_hist[PID_LENGTH-2][1];
    mycar.target_hist[1][1] = mycar.target_hist[PID_LENGTH-1][1];
    for(int i = 2; i < PID_LENGTH; i++) {
      mycar.e_hist[i] = 0;
      mycar.t_hist[i] = 0;
      mycar.motor_hist[i][0] = 0;
      mycar.motor_hist[i][1] = 0;
      mycar.target_hist[i][0] = 0;
      mycar.target_hist[i][1] = 0;
    }
    mycar.e_pos = 2;
    stopmotor();
    mycar.driving = 0;
  ...
  Serial.println(e);
  Serial.print("PID: ");
  Serial.println(KP*e + KD*mycar.dF + KI*mycar.I);*/
  mycar.target_hist[mycar.e_pos][0] = KP*e + KD*mycar.dF + KI*mycar.I;
...
void drivefb(int value) {
  ...
  if (value > 0) {
    ...
    if(sign(mycar.last_drive) != sign(value) && value < 70) {
      ...
      mycar.motor_hist[mycar.e_pos-1][0] = 70;
      mycar.motor_hist[mycar.e_pos-1][1] = 70;
      ...
      analogWrite(MOTORLB, 0);
      analogWrite(MOTORRB, 0);
      mycar.motor_hist[mycar.e_pos-1][0] = value;
      mycar.motor_hist[mycar.e_pos-1][1] = value;
...
void driveturn(int value) {
  ...
  if (value > 0) {
    ...
    if(sign(mycar.last_drive) != sign(value) && value < 140) {
      ...
      mycar.deadtime = millis();
      //Serial.println("Deadband Start");
      mycar.motor_hist[mycar.e_pos-1][0] = 140;
      mycar.motor_hist[mycar.e_pos-1][1] = -140;
    }
    else if(millis()-mycar.deadtime > 30) {
      analogWrite(MOTORLF, value);
...
void setup()
{
  ...
  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Quat6, 1) == ICM_20948_Stat_Ok); // Set to half speed
  ...
  if (tof1.checkForDataReady()) {
    ...
    if(mycar.driving) {
      if(mycar.kalstart) {//normal case, use motor history
        kalman(-(((float)mycar.motor_hist[mycar.e_pos-1][0])/125), dist,true); //Update the kalman filter with this data
      }
    ...
    if(mycar.kalstart) {
      ...
      if(mycar.driving){
        mycar.fronttof = kalman(-((float)mycar.motor_hist[mycar.e_pos-1][0])/125.0, mycar.last_frontof,false);
...
void loop()
{
  ...
  if(mycar.skrt == 1) {
    //state 0, drive until 3 feet, updata orientation unil thn
    if(mycar.state == 0) {
      mycar.driving = 1;
      mycar.dist_target = 1500; //3ft
      get_DMP();
      if(mycar.fronttof < 1500) {
        mycar.driving = 0;
        mycar.state = 1;
        mycar.I = 0;
        ...
        Serial.print("Yaw: ");
        Serial.println(mycar.yaw);

        mycar.orient_target = mycar.yaw+180;
        if(mycar.orient_target > 180) { //indicates its better to go around the other way
          mycar.orient_target -= 360;
        }
        Serial.print("Target: ");
    ...
    else if(mycar.state == 1) {
      float sum = 0;
      if(mycar.e_pos > 10) {
        for (int j =1; j < 11; j++) {
          //will terminate if last 10 readings have less than 5 degrees error and average to less than 0.5 error
        ...
        if(abs(sum) < 2){
          ...
          mycar.state = 2;
          mycar.I = 0;
          mycar.skrttime = millis();
        }
      }
    }
    
    else if(mycar.state == 2) {
      if(millis() - mycar.skrttime > 3000) //3 seconds after turn is done
      {
        mycar.driving = 0;
        mycar.skrt = 0;
        mycar.state = 0;
